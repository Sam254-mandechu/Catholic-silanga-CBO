...
/**
 * Treasurer/admin RPC wrapper: record a donation for a member via the
 * server-side SECURITY DEFINER function `record_donation_for_member`.
 * Falls back to a direct insert if the RPC isn't available.
 */
export async function recordDonationForMember(input: {
  ...
  // Fallback: direct insert (requires appropriate RLS or admin privileges)
  const { data: { user } } = await supabase.auth.getUser();
  const { data, error } = await supabase
    .from('donations')
    .insert({
      ...
      donation_type: input.donation_type ?? ('other' as DonationType),
      message: input.message ?? null,
      member_id: input.member_id,
      donor_id: input.member_id,
      method_id: input.method_id ?? null,
      reference_code: input.reference_code ?? null,
      status: input.status ?? 'completed',
      created_by: user?.id ?? null,
      created_at: input.contribution_date ? new Date(input.contribution_date).toISOString() : new Date().toISOString(),
    })
    .select('*')
    .single();
  if (error) throw error;
...
/**
 * Update an existing donation. Prefers the server-side RPC `update_donation`
 * (security definer) and falls back to a direct UPDATE if the RPC isn't
 * installed.
 */
export async function updateDonation(id: string, patch: Record<string, unknown>): Promise<Donation> {
  // Try RPC
  try {
    const { data, error } = await supabase.rpc('update_donation', { p_donation_id: id, p_patch: patch } as Record<string, unknown>);
    if (!error && data) return data as Donation;
    if (error && !/does not exist/i.test(error.message)) throw error;
  } catch (err: any) {
    if (!/does not exist/i.test(err?.message ?? '')) throw err;
  ...
  // If admin_note explicitly set to null, allow that; otherwise keep as provided.
  const { data, error } = await supabase
    .from('donations')
    .update(updates)
    .eq('id', id)
...
/**
 * Delete a donation. Prefers the server-side RPC `delete_donation` (security
 * definer) and falls back to a direct DELETE.
 */
export async function deleteDonation(id: string): Promise<void> {
  // Try RPC
  try {
    const { error } = await supabase.rpc('delete_donation', { p_donation_id: id } as Record<string, unknown>);
    if (!error) return;
    if (error && !/does not exist/i.test(error.message)) throw error;
  } catch (err: any) {
    if (!/does not exist/i.test(err?.message ?? '')) throw err;
